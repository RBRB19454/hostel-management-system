...
// ======================== UPDATED FOR CLOUDINARY UPLOAD ========================

// Student Registration (multipart form with profileImage)
export const apiRegisterStudent = async (
  data: Omit<StudentProfile,
    'id' | 'role' | 'accountStatus' | 'roomNumber' | 'faculty' | 'year'>
  ...
  if ((data as any).profileImage instanceof File) {
    form.append('profileImage', (data as any).profileImage);
  }
...
// Warden Registration (multipart form: profileImage + wardenIdImage)
export const apiRegisterWarden = async (
  data: Omit<WardenProfile,
    'id' | 'role' | 'accountStatus' | 'hostelManaged'>
): Promise<WardenProfile> => {

  const form = new FormData();
  Object.keys(data).forEach((key) => {
    if (key !== 'profileImage' && key !== 'wardenIdImage') {
      form.append(key, (data as any)[key]);
    }
  });
  ...
  if ((data as any).wardenIdImage instanceof File) {
    form.append('wardenIdImage', (data as any).wardenIdImage);
  }

  const res = await fetch(`${API_URL}/auth/register/warden`, {
    method: 'POST',
    body: form,
  });
...
export const getStudentMaintenanceRequests = (studentId: string): Promise<MaintenanceRequest[]> =>
  apiFetch(`/students/${studentId}/maintenance`);
...
export const submitMaintenanceRequest = (
  req: Omit<MaintenanceRequest, 'id' | 'submittedAt' | 'status' | 'studentName' | 'roomNumber'> & { studentId: string }
): Promise<MaintenanceRequest> =>
  apiFetch('/maintenance', { method: 'POST', body: JSON.stringify(req) });

export const getStudentClearanceStatus = (studentId: string): Promise<ClearanceRequest | null> =>
  apiFetch(`/students/${studentId}/clearance`);
export const applyForClearance = (studentId: string): Promise<ClearanceRequest> =>
  apiFetch(`/students/${studentId}/clearance`, { method: 'POST' });
...
export const postWardenAnnouncement = (
  announcement: Omit<Announcement, 'id' | 'date' | 'author'>,
  authorName: string
...
export const addRoom = (data: Omit<Room, 'id' | 'occupants'>): Promise<Room> =>
  apiFetch('/rooms', { method: 'POST', body: JSON.stringify(data) });
export const updateRoom = (id: string, data: Partial<Room>): Promise<Room> =>
  apiFetch(`/rooms/${id}`, { method: 'PATCH', body: JSON.stringify(data) });
export const deleteRoom = (id: string): Promise<{ success: boolean }> =>
...
export const deleteInventoryItem = (id: string): Promise<{ success: true }> =>
...
export const updateRoomInspectionStatus = (
  clearanceId: string,
  inspectionStatus: 'approved' | 'rejected',
  remarks: string
): Promise<ClearanceRequest> =>
  apiFetch(`/clearance/${clearanceId}/inspection`, {
    method: 'PATCH',
    body: JSON.stringify({ status: inspectionStatus, remarks }),
  });

// Staff Management (Warden)
export const getStaffMembers = (): Promise<StaffMember[]> => apiFetch('/warden/staff');
export const addStaffMember = (data: Omit<StaffMember, 'id' | 'wardenId'>): Promise<StaffMember> =>
  apiFetch('/warden/staff', { method: 'POST', body: JSON.stringify(data) });
export const updateStaffMember = (
  id: string,
  data: Partial<Omit<StaffMember, 'id' | 'wardenId'>>
): Promise<StaffMember> =>
  apiFetch(`/warden/staff/${id}`, { method: 'PATCH', body: JSON.stringify(data) });
export const deleteStaffMember = (id: string): Promise<{ success: true }> =>
  apiFetch(`/warden/staff/${id}`, { method: 'DELETE' });

export const getStaffTasks = (): Promise<StaffTask[]> => apiFetch('/warden/tasks');
export const assignTask = (
  data: Omit<StaffTask, 'id' | 'assignedAt' | 'status' | 'completedAt'>
): Promise<StaffTask> => apiFetch('/warden/tasks', { method: 'POST', body: JSON.stringify(data) });
export const updateTask = (id: string, data: Partial<Omit<StaffTask, 'id' | 'assignedAt'>>): Promise<StaffTask> =>
  apiFetch(`/warden/tasks/${id}`, { method: 'PATCH', body: JSON.stringify(data) });
export const deleteTask = (id: string): Promise<{ success: true }> =>
  apiFetch(`/warden/tasks/${id}`, { method: 'DELETE' });
...
export const postGlobalAnnouncement = (
  announcement: Omit<Announcement, 'id' | 'date' | 'author'>,
  authorName: string
): Promise<Announcement> =>
  apiFetch('/announcements', { method: 'POST', body: JSON.stringify({ ...announcement, author: authorName }) });
...
export const updateAnnouncement = (
  id: string,
  data: Partial<Pick<Announcement, 'title' | 'content' | 'audience'>>
): Promise<Announcement> =>
  apiFetch(`/announcements/${id}`, { method: 'PATCH', body: JSON.stringify(data) });

export const deleteAnnouncement = (id: string): Promise<{ success: true }> =>
...
export const addHostelBlock = (name: string): Promise<HostelBlock> =>
  apiFetch('/settings/blocks', { method: 'POST', body: JSON.stringify({ name }) });
export const updateHostelBlock = (id: string, name: string): Promise<HostelBlock> =>
  apiFetch(`/settings/blocks/${id}`, { method: 'PATCH', body: JSON.stringify({ name }) });
export const deleteHostelBlock = (id: string): Promise<{ success: boolean }> =>
  apiFetch(`/settings/blocks/${id}`, { method: 'DELETE' });
